...
public class Main {
	...
	private static double[] evaluate(Label[] yTrue, Label[] yPred) {
		...
		for(int i = 0; i < yTrue.length; i ++) {
			HW5.printOut(i+1, yTrue[i], yPred[i]);
			if(yTrue[i] == yPred[i]) {
				p++;
			} 
			if(yPred[i] == Label.Negative) {
				if(yTrue[i] == Label.Positive) fn++;
			}
			if(yPred[i] == Label.Positive) {
				if(yTrue[i] == Label.Negative)fp++;
				else if(yTrue[i] == Label.Positive) tp++;
			}
		}
		scores[0] = (double)p/yTrue.length;
		scores[1] = tp/(tp+fp);
		scores[2] = tp/(tp + fn);
		return scores;
	}
